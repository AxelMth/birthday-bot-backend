...
export class ContactMethodService implements ContactMethodUseCase {
  ...
  async getAllContactMethods(): Promise<ContactMethod[]> {
    const contactMethods = await this.contactMethodRepository.getAllContactMethods();
    const uniqueApplicationNames = [...new Set(contactMethods.map((contactMethod) => contactMethod.applicationName))];
    const contactMethodsWithMetadata = await Promise.all(uniqueApplicationNames.map(async (applicationName) => {
      const metadataRepository = MetadataRepositoryFactory.getRepository(applicationName);
    ...
    const metadataByApplicationName = contactMethodsWithMetadata.reduce((acc, metadata) => {
      acc[metadata.applicationName] = metadata.applicationMetadata;
      return acc;
    }, {} as Record<string, Record<string, string>>);
    return contactMethods.map((contactMethod) => ({
      ...contactMethod,
      applicationMetadata: metadataByApplicationName[contactMethod.applicationName],
    }));
  }
}   